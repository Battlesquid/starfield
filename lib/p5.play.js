...
(function(root, factory) {
	...
	function defineLazyP5Property(name, getter) {
		Object.defineProperty(p5.prototype, name, {
			...
			enumerable: true,
			get: function() {
				let context = (this instanceof p5 && !this._isGlobal) ? this : window;
	...
	// For example:
	//
	//   let pInstBind = createPInstBinder(pInst);
	//
	//   let createVector = pInstBind('createVector');
	//   let loadImage = pInstBind('loadImage');
	//
	...
	let abs = p5.prototype.abs;
	let radians = p5.prototype.radians;
	let dist = p5.prototype.dist;
	let degrees = p5.prototype.degrees;
	let pow = p5.prototype.pow;
	let round = p5.prototype.round;
	...
	p5.prototype.createSprite = function(x, y, width, height) {
		let s = new Sprite(this, x, y, width, height);
	...
	p5.prototype._isKeyInState = function(key, state) {
		let keyCode;
		let keyStates = this._p5play.keyStates;
	...
	p5.prototype._isMouseButtonInState = function(buttonCode, state) {
		let mouseStates = this._p5play.mouseStates;
	...
	let now = Date.now();
	let then = Date.now();
	let INTERVAL_60 = 0.0166666; //60 fps
	...
	*/
	...
	function Sprite(pInst, _x, _y, _w, _h) {
		...
		let createVector = pInstBind('createVector');
		let color = pInstBind('color');
		let random = pInstBind('random');
		let print = pInstBind('print');
		let push = pInstBind('push');
		let pop = pInstBind('pop');
		let colorMode = pInstBind('colorMode');
		let noStroke = pInstBind('noStroke');
		let rectMode = pInstBind('rectMode');
		let ellipseMode = pInstBind('ellipseMode');
		let imageMode = pInstBind('imageMode');
		let translate = pInstBind('translate');
		let scale = pInstBind('scale');
		let rotate = pInstBind('rotate');
		let stroke = pInstBind('stroke');
		let strokeWeight = pInstBind('strokeWeight');
		let line = pInstBind('line');
		let noFill = pInstBind('noFill');
		let fill = pInstBind('fill');
		let textAlign = pInstBind('textAlign');
		let textSize = pInstBind('textSize');
		let text = pInstBind('text');
		let rect = pInstBind('rect');
		let cos = pInstBind('cos');
		let sin = pInstBind('sin');
		let atan2 = pInstBind('atan2');

		let quadTree = pInst.quadTree;
		let camera = pInst.camera;


		// These are p5 constants that we'd like easy access to.
		let RGB = p5.prototype.RGB;
		let CENTER = p5.prototype.CENTER;
		let LEFT = p5.prototype.LEFT;
		let BOTTOM = p5.prototype.BOTTOM;
		...
		*/
		...
		this.mouseUpdate = function() {
			...
			let mouseWasOver = this.mouseIsOver;
			let mouseWasPressed = this.mouseIsPressed;
		...
		this.addAnimation = function(label) {
			...
			if (typeof label !== 'string') {
				...
				let newAnimation = sourceAnimation.clone();
		...
		this.overlapPixel = function(pointX, pointY) {
			...
			let img = this.animation.getFrameImage();
			...
			if (point.x < 0 || point.x > img.width || point.y < 0 || point.y > img.height)
				...
				let values = img.get(point.x, point.y);
				return values[3] === 255;
			}
			else {
	...
	function CircleCollider(pInst, _center, _radius, _offset) {
		...
		this.overlap = function(other) {
			...
			r *= r;
			let thisCenterX = this.center.x + this.offset.x;
			let thisCenterY = this.center.y + this.offset.y;
			let otherCenterX = other.center.x + other.offset.x;
			let otherCenterY = other.center.y + other.offset.y;
			let sqDist = pow(thisCenterX - otherCenterX, 2) + pow(thisCenterY - otherCenterY, 2);
			return r > sqDist;
		};
		...
		this.collide = function(other) {
			if (this.overlap(other)) {
				...
				let otherCenterX = other.center.x + other.offset.x;
				let otherCenterY = other.center.y + other.offset.y;
				let a = pInst.atan2(thisCenterY - otherCenterY, thisCenterX - otherCenterX);
				let radii = this.radius + other.radius;
				let intersection = abs(radii - dist(thisCenterX, thisCenterY, otherCenterX, otherCenterY));

				let displacement = createVector(pInst.cos(a) * intersection, pInst.sin(a) * intersection);
	...
	function AABB(pInst, _center, _extents, _offset) {
		...
		this.rotate = function(r) {
			...
			let w2 = this.extents.x * abs(pInst.cos(t)) + this.extents.y * abs(pInst.sin(t));
			let h2 = this.extents.x * abs(pInst.sin(t)) + this.extents.y * abs(pInst.cos(t));
			...
			//box vs circle
				...
				if (distance < other.radius) {
					...
					if (pt.x === other.center.x && pt.y === other.center.y) {
						let xOverlap = pt.x - this.center.x;
						let yOverlap = pt.y - this.center.y;
					...
					let d = createVector(pt.x - other.center.x, pt.y - other.center.y);
					let displacement = createVector(pInst.cos(a) * other.radius - d.x, pInst.sin(a) * other.radius - d.y);
	...
	function Animation(pInst) {
		...
		if (frameArguments.length === 2 && typeof frameArguments[0] === 'string' && typeof frameArguments[1] === 'string') {
			let from = frameArguments[0];
			let to = frameArguments[1];

			//print("sequence mode "+from+" -> "+to);

			//make sure the extensions are fine
			let ext1 = from.substring(from.length - 4, from.length);
			if (ext1 !== '.png') {
				pInst.print('Animation error: you need to use .png files (filename ' + from + ')');
				from = -1;
			}

			let ext2 = to.substring(to.length - 4, to.length);
			if (ext2 !== '.png') {
			...
			if (from !== -1 && to !== -1) {
				...
				let prefix1 = from.substring(0, from.length - (4 + digits1));
				let prefix2 = to.substring(0, to.length - (4 + digits2));
				...
				let number1 = parseInt(from.substring(from.length - (4 + digits1), from.length - 4), 10);
				let number2 = parseInt(to.substring(to.length - (4 + digits2), to.length - 4), 10);
		 ...
		 */
		this.clone = function() {
			let myClone = new Animation(pInst); //empty
			myClone.images = [];
		...
		this.draw = function(x, y, r) {
			...
			if (this.visible) {
				...
				if (this.images[frame] !== undefined) {
					if (this.spriteSheet) {
						let frame_info = this.images[frame].frame;
	 ...
	 *     let player_frames = loadJSON('assets/tiles.json');
	...
	function SpriteSheet(pInst) {
		let spriteSheetArgs = Array.prototype.slice.call(arguments, 1);
		...
		this.drawFrame = function(frame_name, x, y, width, height) {
			...
			let dWidth = width || frameToDraw.width;
			let dHeight = height || frameToDraw.height;
		 ...
		 */
		this.clone = function() {
			let myClone = new SpriteSheet(pInst); //empty

			// Deep clone the frames by value not reference
			for (let i = 0; i < this.frames.length; i++) {
				let frame = this.frames[i].frame;
				let cloneFrame = {
					'name': frame.name,
					'frame': {
						'x': frame.x,
	...
	Quadtree.prototype.retrieveFromGroup = function(pRect, group) {
		...
		let results = [];
		let candidates = this.retrieve(pRect);
